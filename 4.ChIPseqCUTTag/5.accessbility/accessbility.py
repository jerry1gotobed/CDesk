...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Analyze the accessible genome region and plot profile and boxplot")
    
    # Necessary parameters
    parser.add_argument("-bw", "--bigwig", required=True, help="bigwig files list file")
    parser.add_argument("-o", "--output", required=True, help="Output directory")
    parser.add_argument("-m", "--mode", required=True, choices=["region", "gene"], help="Analysis mode：region/gene")
    parser.add_argument("-i", "--input", required=True, help="Input region list file")
    
    # Optional parameters
    parser.add_argument("-s", "--species", help="Species")
    parser.add_argument("--method", choices=["sample", "region_list"], default="sample", 
                        help="boxplot plot method：sample（sample as column）或region_list（region list as column）")
    parser.add_argument("--region_list", help="region list file in region_list mode（multiple genes/regions each row)")
    parser.add_argument("--center", help="average_profile align：peak/position in region mode，TSS/TTS/whole/peak in gene mode")
    parser.add_argument("--promoter_size", type=int, default=5000, help="Gene promoter region size（default：5kb TSS upstream and downstream")
