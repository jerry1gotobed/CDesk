...
os.makedirs(bam_pair_dir, exist_ok=True)
bam_single_dir = os.path.join(outroot,'Bam','single')
os.makedirs(bam_single_dir, exist_ok=True)
...
if 'group' in test.columns:
    df = test
    mask = df['group'].isna() | (df['group'].astype(str).str.strip() == '')
    if mask.any():
        print("NaN or Null in group column")
        sys.exit(1)
    for idx, row in test.iterrows():
        if row['sample'] == row['group']:
            print(f'The sample name {row["sample"]} is the same as the group name, please modify')
            sys.exit(1)
...
for idx, row in test.iterrows():
    ...
    if row['ports'] == 2:
        ...
        if pd.isna(row['fq1']) or pd.isna(row['fq2']):
            ...
            sys.exit(1)
        if os.path.exists(os.path.join(data_dir,sample+'_1.fq.gz')):
            os.remove(os.path.join(data_dir,sample+'_1.fq.gz')) 
        os.symlink(os.path.abspath(row['fq1']),os.path.join(data_dir,sample+'_1.fq.gz'))
        if os.path.exists(os.path.join(data_dir,sample+'_2.fq.gz')):
            os.remove(os.path.join(data_dir,sample+'_2.fq.gz')) 
        os.symlink(os.path.abspath(row['fq2']),os.path.join(data_dir,sample+'_2.fq.gz'))

    if row['ports'] == 1:
        bam_dir = bam_single_dir
        if 'bam' in test.columns:
            if pd.notna(row['bam']):
                if os.path.exists(os.path.join(bam_dir,sample+'.bam')):
                    os.remove(os.path.join(bam_dir,sample+'.bam')) 
        ...
        if pd.notna(row['fq1']):
            if os.path.exists(os.path.join(data_dir,sample+'_single.fq.gz')):
                os.remove(os.path.join(data_dir,sample+'_single.fq.gz')) 
            os.symlink(os.path.abspath(row['fq1']),os.path.join(data_dir,sample+'_single.fq.gz'))
        else:
            if os.path.exists(os.path.join(data_dir,sample+'_single.fq.gz')):
                os.remove(os.path.join(data_dir,sample+'_single.fq.gz')) 
